...
- **Layout**: `src/` layout (`src/geotoolz/`)
- **Testing**: pytest
...
**All four checks must pass before any commit.** CI runs them on the entire repo (`ruff check .`), not just `src/geotoolz/`, so always run the commands below from the repo root.
...
> **Common pitfall**: Running `ruff check src/geotoolz/` instead of `ruff check .` misses import-sorting errors in `tests/` and `scripts/`. The CI workflow runs `ruff check .`. Always use `.` (repo root), not a subdirectory.
